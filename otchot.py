...
**{i}.** {status_icon} **{product_short}**
   ...
   📅 Sana: {submission_date}"""
		...
		profile_text += "n❌ Hozircha hisobotlar yo'q"
...
async def start_report_submission(message: Message, state: FSMContext, bot: Bot):
	...
	if await check_user_blocked(user_id):
		await message.answer(
			"🚫 Sizning hisobingiz vaqtincha bloklangan.n"
	...
	if not assigned_group:
		await message.answer(
			"⚠️ Sizga hali guruh tayinlanmagan.n"
	...
	await state.clear()
	sent_message = await message.answer(
		"👤 Mijozning to'liq ismini kiriting:nn"
...
async def process_client_name(message: Message, state: FSMContext, bot: Bot):
	...
	if not validate_text_field(client_name, 3):
		await show_error_and_retry(
			message, state, bot,
			"⚠️ Iltimos, mijozning to'liq ismini kiriting (kamida 3 belgi).nn"
			"💡 Masalan: Abdullayev Akmal Akbarovichnn"
			"Qaytadan kiriting:"
		)
		return
	...
	await process_step(
		...
		"📱 Mijozning telefon raqamini kiriting:nn"
...
async def process_phone_number(message: Message, state: FSMContext, bot: Bot):
	...
	if not validate_phone_number(phone_number):
		await show_error_and_retry(
			message, state, bot,
			"⚠️ Iltimos, to'g'ri telefon raqamini kiriting.nn"
			"💡 Masalan: +998901234567 yoki 998901234567nn"
			"Qaytadan kiriting:"
		)
		return
	...
	await process_step(
		...
		"📱 Mijozning qo'shimcha telefon raqami bormi?nn"
...
async def ask_additional_phone(callback_query: CallbackQuery, state: FSMContext, bot: Bot):
	...
	await callback_query.message.edit_text(
		"📱 Qo'shimcha telefon raqamini kiriting:nn"
		"💡 Masalan: +998901234567 yoki 998901234567",
		reply_markup=get_cancel_report_inline_keyboard()
	)
...
async def process_product_type(message: Message, state: FSMContext, bot: Bot):
	...
	if not validate_text_field(product_type, 2):
		await show_error_and_retry(
			message, state, bot,
			"⚠️ Iltimos, mahsulot nomini to'g'ri kiriting (kamida 2 belgi).nn"
			"💡 Masalan: Samsung Galaxy A54 128GBnn"
			"Qaytadan kiriting:"
		)
		return
	...
	await process_step(
		...
		"📍 Mijozning to'liq manzilini kiriting:nn"
...
async def process_client_location_text(message: Message, state: FSMContext, bot: Bot):
	...
	if not validate_text_field(client_location_text, 10):
		await show_error_and_retry(
			message, state, bot,
			"⚠️ Iltimos, mijoz manzilini to'liqroq kiriting (kamida 10 belgi).nn"
			"💡 Masalan: Toshkent shahar, Chilonzor tumani, Bunyodkor ko'chasi 12-uynn"
			"Qaytadan kiriting:"
		)
		return
	...
	await process_step(
		...
		"📄 Shartnoma ID raqamini kiriting:nn"
...
async def process_contract_id(message: Message, state: FSMContext, bot: Bot):
	...
	if not validate_text_field(contract_id, 3):
		await show_error_and_retry(
			message, state, bot,
			"⚠️ Iltimos, shartnoma ID raqamini kiriting (kamida 3 belgi).nn"
			"💡 Masalan: SH-2024-001 yoki 240115001nn"
			"Qaytadan kiriting:"
		)
		return
	...
	await process_step(
		...
		"💰 Shartnoma summasini kiriting:nn"
		"💡 Masalan: 5000000 yoki 5,000,000 yoki 5.000.000n"
...
async def process_contract_amount(message: Message, state: FSMContext, bot: Bot):
	...
	if not contract_amount_raw or not any(char.isdigit() for char in contract_amount_raw):
		await show_error_and_retry(
			message, state, bot,
			"⚠️ Iltimos, shartnoma summasini kiriting.nn"
			"💡 Masalan: 5000000 yoki 5,000,000 yoki 5.000.000nn"
			"Qaytadan kiriting:"
		)
		return
	...
	if formatted_amount != contract_amount_raw:
		try:
			confirmation_message = await message.answer(
				f"💰 Kiritilgan summa: {contract_amount_raw}n"
				f"📊 Formatlangan summa: {formatted_amount}nn"
	...
	await process_step(
		...
		"🖼️ Mahsulot rasmini yuboring:nn"
...
async def process_product_image(message: Message, state: FSMContext, bot: Bot):
	...
	if user_data.get('additional_phone_number'):
		additional_phone_text = f"n📱 Qo'shimcha telefon: {user_data.get('additional_phone_number')}"
...
👤 Mijoz: {user_data.get('client_name', 'Nomalum')}
📱 Telefon: {user_data.get('phone_number', 'Nomalum')}{additional_phone_text}
🛍️ Mahsulot: {user_data.get('product_type', 'Nomalum')}
📍 Manzil: {user_data.get('client_location', 'Nomalum')}
📄 Shartnoma ID: {user_data.get('contract_id', 'Nomalum')}
💰 Shartnoma summasi: {user_data.get('contract_amount', 'Nomalum')} so'm
...
async def incorrect_product_image(message: Message, state: FSMContext, bot: Bot):
	...
	await show_error_and_retry(
		message, state, bot,
		"⚠️ Iltimos, faqat rasm yuboring!nn"
		"💡 JPG yoki PNG formatidagi rasm yuborishingiz kerak.nn"
...
async def confirm_report_submission(callback_query: CallbackQuery, state: FSMContext, bot: Bot):
	...
	additional_phone_line = ""
	if user_data.get('additional_phone_number'):
		additional_phone_line = f"📱 Qo'shimcha telefon: {user_data.get('additional_phone_number')}n"
	
	# Guruh uchun hisobot matnini tayyorlash
	status_line = "Holati: ⏳ Kutilmoqda"
	report_caption = REPORT_CAPTION_TEMPLATE.format(
		client_name=user_data.get('client_name', 'Nomalum'),
		phone_number=user_data.get('phone_number', 'Nomalum'),
		additional_phone_line=additional_phone_line,
		product_type=user_data.get('product_type', 'Nomalum'),
		client_location=user_data.get('client_location', 'Nomalum'),
		contract_id=user_data.get('contract_id', 'Nomalum'),
		contract_amount=user_data.get('contract_amount', 'Nomalum') + " so'm",
	...
	try:
		...
		await callback_query.message.edit_caption(
			caption="✅ Hisobotingiz muvaffaqiyatli yuborildi!nn"
			        "🎯 Hisobotingiz tekshirilish uchun yuborildi.n"
			        "📊 Tasdiqlangandan so'ng Google Sheets'ga saqlanadi.",
			reply_markup=None
		)
		
		await callback_query.message.answer(
			f"🎉 Hisobotingiz '{group_name}' guruhiga yuborildi!nn"
			f"📋 Hisobot ID: #{report_id}n"
			f"⏰ Yuborilgan vaqt: {datetime.now().strftime('%d.%m.%Y %H:%M')}nn"
...
async def edit_report(callback_query: CallbackQuery, state: FSMContext, bot: Bot):
	...
	await callback_query.message.edit_caption(
		caption="✏️ Qaysi ma'lumotni o'zgartirmoqchisiz?nn"
...
async def cancel_report_final(callback_query: CallbackQuery, state: FSMContext, bot: Bot):
	...
	await state.clear()
	await callback_query.message.edit_caption(
		caption="🚫 Hisobot bekor qilindi.nn"
...
async def handle_edit_selection(callback_query: CallbackQuery, state: FSMContext, bot: Bot):
	...
	edit_configs = {
		"client_name": {
			...
			"prompt": "👤 Mijozning yangi ismini kiriting:nn💡 Masalan: Abdullayev Akmal Akbarovich"
		},
		"phone": {
			"state": ReportState.waiting_for_phone_number,
			"prompt": "📱 Yangi telefon raqamini kiriting:nn💡 Masalan: +998901234567"
		},
		"additional_phone": {
			"state": ReportState.waiting_for_additional_phone_number,
			"prompt": "📱 Yangi qo'shimcha telefon raqamini kiriting:nn💡 Masalan: +998901234567"
		},
		"product": {
			"state": ReportState.waiting_for_product_type,
			"prompt": "🛍️ Yangi mahsulot nomini kiriting:nn💡 Masalan: Samsung Galaxy A54 128GB"
		},
		"location": {
			"state": ReportState.waiting_for_client_location_text,
			"prompt": "📍 Yangi manzilni kiriting:nn💡 Masalan: Toshkent shahar, Chilonzor tumani"
		},
		"contract_id": {
			"state": ReportState.waiting_for_contract_id,
			"prompt": "📄 Yangi shartnoma ID'sini kiriting:nn💡 Masalan: SH-2024-001"
		},
		"contract_amount": {
			"state": ReportState.waiting_for_contract_amount,
			"prompt": "💰 Yangi shartnoma summasini kiriting:nn💡 Masalan: 5000000 yoki 5,000,000"
		},
		"image": {
			"state": ReportState.waiting_for_product_image,
			"prompt": "🖼️ Yangi mahsulot rasmini yuboring:nn💡 Faqat rasm formatida (JPG, PNG)"
		}
	}
...
async def confirm_report_handler(callback_query: CallbackQuery, bot: Bot):
	...
	updated_caption = "n".join(filtered_lines)
		...
		if sender_name:
			...
			if sender_telegram_id:
				await bot.send_message(
					...
					text=f"❌ Sizning hisobotingiz rad etildi.nn"
...
async def contact_helper(callback_query: CallbackQuery, bot: Bot):
	...
	await callback_query.message.edit_text(
		f"📞 Hisobotingiz rad etilish sababi haqida ma'lumot olish uchun helper bilan bog'laning:nn"
...
async def view_seller_info(callback_query: CallbackQuery, bot: Bot):
	...
	if not seller_telegram_id:
		await callback_query.answer(f"👨‍💼 Sotuvchi: {seller_name}n❌ Profil ma'lumotlari topilmadi.", show_alert=True)
		return
	...
	try:
		...
		if not profile_data:
			await callback_query.answer(f"👨‍💼 Sotuvchi: {seller_name}n❌ Profil ma'lumotlarini olishda xatolik.",
			                            show_alert=True)
			return
